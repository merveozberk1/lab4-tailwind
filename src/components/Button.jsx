export default function Button({
  ...
  // Temel buton özellikleri (her butonda ortak)
  const base = "inline-flex items-center justify-center font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2";

  // Renk ve tema varyantları (Karanlık mod destekli)
  const variants = {
  ...
  return (
    <button
      className={`${base} ${variants[variant]} ${sizes[size]} ${disabled ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}
      disabled={disabled}
      {...props}
    >
      {children}
